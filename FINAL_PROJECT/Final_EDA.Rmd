...
First let us take a look at the new dataset with its new variables:
```{r}
...
Since this is our best model, lets try to predict a few made up New US states:
...
This model still is not a great predictor of statewide body camera usage, which can lead us to the following conclusions:


##EDA (ALEX help us with the interpretation of the following graphs)
...
fig <- plot_ly(FD, x = FD$bclaw, y = FD$stbcp, z = FD$marg2020,color = factor(FD$bclaw), colors = c('red', 'black'))
fig <- fig %>% add_markers()
fig <- fig %>% layout(scene = list(xaxis = list(title = 'police officers to turn on their cameras'),
                     yaxis = list(title = 'proportion of fatal shootings'),
                     zaxis = list(title = 'percentage margin')))
fig
```


The following 3D plot compares the following variables:
regions,stbcp,marg2020
...
Histogram on stbcp
```{r}
ggplot(FD, aes(x=stbcp)) + geom_histogram( binwidth =0.03,aes(fill = ..count..))+scale_fill_gradient('Count',low='blue',high='red') + labs(x = "proportion of fatal shootings that were caught on camera by police", y="Number of incidents captured on camera", title = "Histogram")
```
Histogram on spendpc
```{r}
