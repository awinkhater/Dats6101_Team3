...
```
```{r}
lm2 <- lm(stbcp~ (marg2020+bclaw+spendpc), data=FD)
summary(lm2)
car::vif(lm2)
```
```{r}
lm3 <- lm(stbcp~ (bclaw + spendpc), data=FD)
summary(lm3)
```
```{r}
#Redo of lm1 but with an interaction
lm4 <- lm(stbcp~ (marg2020 + bclaw + regions + le_per_100k+ spendpc + (spendpc*le_per_100k)), data=FD)
summary(lm4)
car::vif(lm4 )
```
```{r}
lm5 <- lm(stbcp~ (bclaw + le_per_100k+ spendpc + (spendpc*le_per_100k)), data=FD)
summary(lm5)
car::vif(lm5 )
```
