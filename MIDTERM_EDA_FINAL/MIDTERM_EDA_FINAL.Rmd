...
We also create some new columns for the Modeling Portion 
```{r}
...
```
Here we are adding data about the direction a state swung in the 2020 election (Negative = Swing to Trump, Positive = Swing to Biden)
```{r}
...
```
Finally We will add a variable based on police officers per 100K citizens by state
```{r}
...
```
```{r}
lm1 <- lm(stbcp~ (marg2020 + bclaw + regions), data=FD)
summary(lm1)
car::vif(lm1 )
```
```{r}
lm2 <- lm(stbcp~ (marg2020), data=FD)
summary(lm2)

```
```{r}
lm3 <- lm(stbcp~ (bclaw + regions), data=FD)
summary(lm3)
```
```{r}
lm4 <- lm(stbcp~ (spendpc+ bclaw ), data=FD)
summary(lm4)
car::vif(lm4 )
```
#THESE MODELS ARE NOT UP TO DATE PLEASE CHANGE THEM AND EXPERIMENT WITH RELEVANT VARIABLES