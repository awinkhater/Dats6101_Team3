...
```
```{r}
lm1 <- lm(stbcp~ (spendpc + bclaw + regions + month), data=FD)
summary(lm1)
car::vif(lm1 )
```
```{r}
lm2 <- lm(stbcp~ (spendpc), data=FD)
summary(lm2)
...
```
#Swing 2020 Variable ??
#Police Officer per state variable ??
#(These are hard as hell to add, but I can do it)
